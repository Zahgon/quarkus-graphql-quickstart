...
/** DDL names for the column types whose spelling does not depend on a length. */
const FIXED_DDL_TYPES = { integer: 'INTEGER', date: 'DATE' } as const;

function varchar(type: ColumnType, length: number | undefined): string {
  if (type === 'string') {
    return length === undefined ? 'VARCHAR(255)' : `VARCHAR(${String(length)})`;
  }
  return FIXED_DDL_TYPES[type];
}
