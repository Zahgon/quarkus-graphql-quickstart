...
export const PROFILE_DEV: Profile = 'dev';
export const PROFILE_TEST: Profile = 'test';
export const PROFILE_PROD: Profile = 'prod';

function activeProfile(): Profile {
  const explicit = process.env['QUARKUS_PROFILE'];
  if (explicit === PROFILE_PROD || explicit === PROFILE_DEV || explicit === PROFILE_TEST) {
    return explicit;
  }
  if (process.env['NODE_ENV'] === 'production') {
    return PROFILE_PROD;
  }
  if (process.env['VITEST'] !== undefined || process.env['NODE_ENV'] === PROFILE_TEST) {
    return PROFILE_TEST;
  }
  return PROFILE_DEV;
}
